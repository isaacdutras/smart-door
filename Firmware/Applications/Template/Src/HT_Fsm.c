...
static const char clientID[] = {"SIP_HTNB32L-XXX-ISAAC"};
...
char topic_buzzer[] = {"hana/prototipagem/smartdoor/buzzer"};

// MQTT Topics to Publish
char topic_lux[] = {"hana/prototipagem/smartdoor/lux"};
char topic_switch[] = {"hana/prototipagem/smartdoor/door"};
...
static StaticTask_t buzzer_thread;
static uint8_t yieldTaskStack[1024*4];
static StaticTask_t lux_thread;
static uint8_t luxTaskStack[1024*4];
static StaticTask_t switch_thread;
 ...
 * \retval none.
 *******************************************************************/
static void HT_LuxThread(void *arg);
 ...
 * \retval none.
 *******************************************************************/
static void HT_Lux_Thread_Start(void *arg);
...
static void HT_LuxThread(void *arg) {
    ...
    while (1) {
        ...
        uint16_t luxValue = lightSensor_meter();
        uint8_t luxPayload[20] = {0};
        sprintf((char *)luxPayload,"%u",luxValue);
        printf("Lux Value: %dmv\n", luxValue);
        printf("Publishing Lux on/off...\n");
        
        // TAREFA EXTRA: Implementar mutex aqui para evitar chamar o HT_MQTT_Publish em dois lugares simultaneamente.
        HT_MQTT_Publish(&mqttClient, topic_lux, luxPayload, strlen((char *)luxPayload), QOS0, 0, 0, 0);
...
static void HT_Yield_Thread_Start(void *arg) {
    ...
    memset(yieldTaskStack, 0xA5,LUX_TASK_STACK_SIZE);
    task_attr.name = "yield_thread";
    task_attr.stack_mem = yieldTaskStack;
    task_attr.stack_size = LUX_TASK_STACK_SIZE;
...
static void HT_Lux_Thread_Start(void *arg) {
    ...
    memset(&task_attr,0,sizeof(task_attr));
    memset(ldrTaskStack, 0xA5,LUX_TASK_STACK_SIZE);
    task_attr.name = "ldr_thread";
    task_attr.stack_mem = ldrTaskStack;
    task_attr.stack_size = LUX_TASK_STACK_SIZE;
    task_attr.priority = osPriorityBelowNormal6;
    task_attr.cb_mem = &lux_thread;
    task_attr.cb_size = sizeof(StaticTask_t);

    osThreadNew(HT_LuxThread, NULL, &task_attr);
}
